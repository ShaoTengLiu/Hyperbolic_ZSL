...
##################################################
    ...
    #     opt.gpu = -1
    #     log.warning('Specified hogwild training with GPU, defaulting to CPU...') # f
##################################################

    # set default tensor type
    th.set_default_tensor_type('torch.DoubleTensor')
    # set device
    device = th.device('cuda:{}'.format(opt.gpu) if opt.gpu >= 0 else 'cpu')
    ...
    log.info('json_conf: {}'.format(json.dumps(vars(opt))))
    ...
    def control(model, epoch, elapsed, loss):
        ...
        checkpoint.path = '{}.{}'.format(opt.checkpoint, epoch)
        checkpoint.save({
        ...
        while not logQ.empty():
            ...
            log.info('json_stats: {}'.format(json.dumps(lmsg)))
